...
namespace ASP.Middleware.Cart
{
    public class UserCartMiddleware(RequestDelegate next)
    {
        ...
        public async Task InvokeAsync(HttpContext context, DataAccessor dataAccessor)
        {
            ...
            {
                string userId = context.User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.PrimarySid)!.Value;
                context.Items.Add(nameof(UserCartModel), new UserCartModel()
                {
    ...
    {
        public static IApplicationBuilder UseUserCart(this IApplicationBuilder builder)
        {
